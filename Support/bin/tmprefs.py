...
class Preferences(object):
    ...
    def readTMPrefs(self):
        """readTMPrefs reads the textmate preferences file and constructs a
        python dictionary.
        ...
        latexViewer = Skim
        """
        return NSDictionary.dictionaryWithContentsOfFile_(
            os.environ["HOME"]+"/Library/Preferences/" + TM_PREFERENCE_FILE)

    def toDefString(self):
        """docstring for toDefString"""
        instr = plistlib.writePlistToString(self.defaults)
        runObj = Popen('pl', shell=True, stdout=PIPE, stdin=PIPE,
